	...
	<div align="center">
		...
		<c:if test="${id == 'admin' }">
			<button type="button" class="btn_write" 
				onclick="location.href='map_event_admin.do'">
		...
		<p class="event_text"><b>CLOSE</b></p>
		<br> <br>
		
		<table border="0" cellspacing="0" width="500">
			
			<c:set var="list" value="${endList }" />
			...
			<c:if test="${!empty list }">
				<c:forEach items="${list }" var="i">
					<tr>
						<td align="center">
							<img src="<%=request.getContextPath() %>/resources/upload/${i.getBpath() }" height="200" width="180"><br>
							<a class="event_a" href="<%=request.getContextPath() %>/event_content.do?bno=${i.getBno() }&page=${dto.getPage() }">
								${i.getBtitle() }</a>
							<hr width="500" color="#D3D3D3">
						</td>
					</tr>
				</c:forEach>
			</c:if>
		...
		<c:if test="${dto.getPage() > dto.getBlock() }">
	      <a class="page_a" href="end_event_list.do?page=1">◀◀</a>
	      <a class="page_a" href="end_event_list.do?page=${dto.getStartBlock() - 1 }">◀</a>
	   </c:if>
	   
	   <c:forEach begin="${dto.getStartBlock() }"
	                   end="${dto.getEndBlock() }" var="i">
	      <c:if test="${i == dto.getPage() }">
	         <b> <a class="page_a" href="end_event_list.do?page=${i }">[${i }]</a></b>
	      </c:if>
	      
	      <c:if test="${i != dto.getPage() }">
	         <a class="page_a" href="end_event_list.do?page=${i }">[${i }]</a>
	      </c:if>
	   </c:forEach>
	   
	   <c:if test="${dto.getEndBlock() < dto.getAllPage() }">
	       <a class="page_a" href="end_event_list.do?page=${dto.getEndBlock() + 1}">▶</a>
	      <a class="page_a" href="end_event_list.do?page=${dto.getAllPage() }">▶▶</a>
	   </c:if>
	</div>
